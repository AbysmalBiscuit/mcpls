...
- **`server sees:`** what the running mcpls, if any, reports back. A healthy line repeats the hook's own directory and hash, as in the example above, confirming both sides agree. When no owner answers this project's own socket, the line takes one of four shapes instead:
  ...
  When an owner does answer but the exchange itself fails, the line reports the specific fault instead, for example an owner answering with an error message, answering with something other than its own status (most likely a version mismatch between the hook and the server), a socket that accepted the connection but whose reply this build could not parse, or a socket that answered nothing within the probe window. Each of those pairs with `owner pid: unknown` below; the printed detail says what to check.
