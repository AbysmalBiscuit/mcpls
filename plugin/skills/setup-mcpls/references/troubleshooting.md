...
| "no LSP server configured for language: ..." | No `[[lsp_servers]]` entry covers the file's language. Add one, or map the extension in `workspace.language_extensions`. `mcpls schema` describes both. |
...
| `--listen` is an unknown argument, or `MCPLS_LISTEN` is ignored | The binary lacks `transport-http`: `--listen` is a parse error, and the variables are ignored. Reinstall with the feature; see [install.md](install.md). |
