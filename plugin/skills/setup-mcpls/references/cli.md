...
- `mcpls status` lists every backend the user runs, across all checkouts: its checkout root, pid, version, attached sessions, language servers grouped by state, and log path. Checkouts with no backend are left out. A backend that refuses to report, such as one from another mcpls build or with hooks switched off, appears with an unknown root and the reason. It exits 0 whether or not any backend runs.
