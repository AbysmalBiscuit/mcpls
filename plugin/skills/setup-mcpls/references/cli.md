...
- `mcpls lsp install <SERVER>... | --all [--dir DIR] [--dry-run]` runs each server's configured `install` command, one at a time, in the checkout root, and prints one line per server. It skips a server whose `command` already resolves and one with no install command for this OS. It needs no backend. `--dry-run` prints each command and runs nothing. Exits non-zero when a command fails, a binary still does not resolve afterwards (an installer that changes `PATH` only for new shells), a server named here has no install command, or a name does not apply.
