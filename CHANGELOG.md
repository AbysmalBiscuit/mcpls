...
* **release:** publish fork binaries and installers ([#47](https://github.com/AbysmalBiscuit/mcpls/issues/47)) ([870cbaf](https://github.com/AbysmalBiscuit/mcpls/commit/870cbafc5d481985dd6a80605bb3d8465332d453))
...
- **`diagnostics_pump` now drops a `PublishDiagnostics` naming a version below the one `DocumentTracker` currently holds for that path** — a publish with no version, or for a path the tracker holds no entry for, is still kept, since those are exactly the fanout files a server publishes without ever being asked to open them. `PumpShared` carries an `Arc<DocumentTracker>` for the lookup, via a new `DocumentTracker::version_of` that reads just the tracked version rather than cloning the whole document. Has no observable effect yet: applying an edit today closes every document it wrote, so a reopen restarts at version 1 and there is nothing yet for a stale publish to be older than.
- **A diagnostics baseline is taken once every language server has stayed quiet** — new `ServerSettle` tracks outstanding `$/progress` operations per server; the workspace counts as settled once the outstanding count has held at zero for `diagnostics.settle_quiet_ms`, or once `diagnostics.settle_deadline_ms` passes regardless, whichever comes first. The deadline is counted from the moment the language servers are spawned, not from process start, so it bounds indexing rather than being spent on config load and every server's `initialize` handshake; it defaults to five minutes, chosen to outlast a full index of a large workspace, since a backstop that fires early captures a partial baseline and reports the rest of the workspace as newly changed, while one that fires late only makes a session wait longer for a first baseline it is told to wait for. The first zero-crossing is not enough on its own: rust-analyzer's startup phases hand off to each other through brief gaps of outstanding work well before indexing (let alone the first `cargo check`) finishes, so a latch on the first crossing would baseline an empty workspace. A background task spawned alongside the diagnostics pumps polls for settling and then adopts the current `NotificationCache` contents as `DiagnosticsDelivery`'s baseline via a new `NotificationCache::diagnostics_snapshot`. `PumpShared` gains `settle`, `delivery`, and `floors` fields, and the diagnostics pump now feeds `ServerSettle` from `$/progress` `begin`/`end` notifications. Nothing calls `DiagnosticsDelivery::flush` yet, so the baseline has no observable effect so far.
- **mcpls now advertises `window.workDoneProgress` and answers `window/workDoneProgress/create`** — every language server mcpls spawns is told about this capability during the `initialize` handshake, and a server asking to open a progress token is acknowledged instead of getting `-32601`. Without both, a server must not send `$/progress` at all, which is why the settle debounce above could never actually fire: `ServerSettle` never saw any outstanding operation, so only `settle_deadline_ms` ever landed a baseline. Confirmed against a real rust-analyzer: a baseline now lands within `settle_quiet_ms` of startup going quiet, typically a few seconds, instead of waiting out the full deadline.
- **`get_new_diagnostics` MCP tool** — takes no parameters and returns the diagnostics that changed since the caller last asked: `changed` (each file's path, its diagnostics at or above the configured severity floor, and how many admitted diagnostics the volume caps held back), `cleared` (paths that had diagnostics before and now have none), and a whole-flush `omitted` count for files the total budget could not fit this call. Deduplicated per client session (`SessionId`, keyed off `CLAUDE_CODE_SESSION_ID` where the host sets it), so calling repeatedly with no intervening edits returns nothing. Because draining advances that per-session record, the tool advertises `readOnlyHint: false`, `destructiveHint: false` and `idempotentHint: false` rather than inheriting mcpls' read-only default — a host that dedupes or retries a call it believes to be read-only and idempotent would otherwise discard a response whose diagnostics are already gone. Positions are converted through the same encoding path `get_cached_diagnostics` uses, so the two tools never disagree about a column. Before the workspace's diagnostics baseline exists (see the settle entry above), the tool reports empty `changed`/`cleared` plus a `note` explaining the language servers are still starting up, rather than draining anything — draining early would seed the session's record from an empty view and make the workspace's own pre-existing diagnostics look newly changed on every later call. A workspace with no applicable LSP server at all adopts an empty baseline immediately, since nothing would otherwise ever settle. The same `note` field also appears whenever the top-level `omitted` count is non-zero, saying how many files the caps held back and that a later call offers them again — a bare count on its own doesn't tell a caller that a retry gets the rest. This is the first caller for the `[diagnostics]` config table, the stale-publish guard, and the settle-driven baseline landed above: all three were inert until now.
- **`mcpls completions <shell>` subcommand** — prints a shell completion script to stdout for bash, elvish, fish, nushell, powershell, and zsh. The script is generated from the live `Args` definition, so it covers whatever flags the build accepts: a build without the `transport-http` feature emits completions without `--listen`/`--http-path`. The shell argument is a first-party `completions::Shell` rather than `clap_complete::Shell`, which is closed and has no nushell variant; each variant forwards to the crate owning that generator (`clap_complete` for five, `clap_complete_nushell` for nushell), and the value strings match `clap_complete::Shell`'s so they stay stable. The script is rendered into memory and then written to stdout in a single write, because the generators panic on a failed write rather than reporting it — `clap_complete::generate` at the top level, and fish's `try_generate` through a nested `expect` of its own — which would turn `mcpls completions fish | head` into a crash; that single write treats a broken pipe as the reader being done. This is the binary's first subcommand: `mcpls` with no subcommand still runs the MCP server exactly as before, and the completions path short-circuits before config loading and logging initialization. Documented under "Shell Completions" in `docs/user-guide/installation.md`.

### Changed

- **`[[lsp_servers]]` config entries now merge onto the six built-in servers instead of replacing the whole list** — Breaking change: previously, writing any `[[lsp_servers]]` entry discarded every built-in not restated, so reaching one per-server field (a timeout, a heuristic) cost every other default server, and a config file with no `[[lsp_servers]]` table at all spawned nothing. Each entry now folds onto the built-in sharing its routing identity (`name` if set, else `language_id`): a field the entry omits is inherited from the built-in, a field it sets overrides. Overriding `command` also drops the built-in's `args`, `env`, and `initialization_options`, since those belong to the binary being replaced — `file_patterns` survive, since they describe the language rather than the binary. New `enabled = false` field removes every server sharing an entry's identity, replacing the disable-by-omission that whole-list replacement gave for free. Only the *first* entry claiming an identity merges; a later one with the same identity appends another server, so two servers for one language separated by mutually exclusive `heuristics.project_markers` still load as two servers rather than one overwriting the other. New `LspServerConfig::builtins()` and `PartialLspServerConfig`/`resolve_lsp_servers` (both newly `pub`) back this. Existing configs that named every built-in field-for-field are unaffected; a config that relied on an `[[lsp_servers]]` entry silently disabling every other built-in now needs `enabled = false` for the ones it doesn't want. The loudest breakage: a config that gave a built-in language's server an explicit `name` (e.g. `name = "pyright"` on a `python` entry) now fails at startup with "two catch-all servers" for that language, since the entry no longer matches the built-in's identity and both are unconditionally applicable — add `enabled = false` for that built-in, or drop the `name`.
...
- **Four residual `workspace.roots` resolution edge cases from #345** — an all-absolute `workspace.roots` no longer requires a successful `current_dir()`/`config_dir` canonicalization to start `mcpls` or load a config file; the auto-discovered global/user config (`~/.config/mcpls/mcpls.toml`) now resolves a relative root against the process cwd instead of its own directory, matching pre-#345 behavior; a Windows root with no drive/UNC prefix (e.g. `\workspace`) or a drive-relative root (e.g. `C:workspace`) now joins correctly under the base directory instead of silently discarding it; and an empty `workspace.roots` entry (`roots = [""]`), which previously loaded silently and resolved to the base directory, is now rejected by `validate()` with a clear invalid-configuration error. (#351)
