...
- **`diagnostics_pump` drops a publish naming a version below the tracked one** — a language server publishing diagnostics for a version of a document that a subsequent edit has already replaced previously overwrote the cache, showing the agent errors for text that no longer exists until the next publish corrected them. The pump now consults the workspace's `DocumentTracker` and drops a `PublishDiagnostics` whose version is older than the one currently tracked for that path. A publish naming no version, or naming a path the tracker holds no entry for, is still kept: those are exactly the fanout files a server publishes without ever being asked to open them. Applying an edit today closes every document it wrote, which resets the tracked version back to 1 on the next open, so this check has no observable effect yet; it starts mattering once edits stop closing the document they touch.
