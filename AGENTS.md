...
Commit through `devrun task commit`, which renders the Conventional Commits subject, wraps the body, and appends the co-author trailers. An agent's commit ends with a trailer naming the model that wrote it, `Co-authored-by: AGENT MODEL <ADDRESS>`:
