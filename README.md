...
Codex hook `agent_id` and MCP thread metadata identify the same delivery record. Claude hooks use `agent_id`; Claude MCP calls without an agent marker read the root record. Filesystem watcher events refresh diagnostics without claiming authorship.
