...
5. Write a durable dated evidence note linking commits, exact CI run/job artifacts, behavioral tests, and any unavailable history. Snapshot counts are allowed here; include reproduction commands for evidence that can be regenerated. Update the issue checklist/project status only after current requirements are accounted for; the PR references #7 while original comparison is pending and closes it only once requirements are verified.
...
## Task 4: Wait for actual readiness in the shutdown fixture

**Files:** `crates/mcpls-core/tests/e2e/mcp_client.rs` and `crates/mcpls-core/tests/e2e/protocol_tests.rs`. Production behavior remains unchanged.

1. Consume the observed assertion failure in `.devkit/final-logs/other-e2e.log`: the pre-handshake SIGTERM test received signal termination instead of exit status zero. The unchanged isolated test passed, and source ordering confirms its fixed 50 ms delay does not establish child readiness. Treat this as fixture timing evidence, not a new production regression.
2. Add a focused test-client spawn path that pipes and continuously drains child stderr, relays logs for failed-test diagnostics, and waits at most five seconds for the existing `Listening for MCP requests on stdio...` marker. The normal spawn path keeps its behavior. Reuse the existing constructor rather than duplicating process setup. Hold client ownership while waiting so early error drops kill/reap the child; keep any reader lifetime bounded by child cleanup.
3. Replace only the pre-handshake test's fixed sleep with this readiness wait. Send SIGTERM immediately afterward without sending initialize. Preserve the existing exit-code assertion and shutdown deadline. The marker follows persistent signal registration and precedes the stdio handshake, so the test still detects failure to handle SIGTERM during initialization.
4. Replace stale comments attached to this test with concise explanations of the readiness invariant. Do not perform a broader comment sweep, add production test hooks or new protocol fields, or change external timeouts.
5. Run focused `issue7-sigterm`, then `issue7-other-e2e`, formatting and Clippy using the supplied devkit config/environment. No full workspace or RA repeat is needed for this fixture-only correction. Do not run repeated stress loops or rebuild historical revisions. Record the original failure, unchanged focused pass, final checks and limitations in `task-4-report.md`; commit the test correction and this plan update selectively after self-review.
