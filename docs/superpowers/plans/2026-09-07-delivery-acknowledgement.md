...
> The session record only ever takes a hash from a report whose answer a reader confirmed receiving, or that the tool or footer door put into its own response. `stage` writes nothing to the record. `commit` writes exactly the updates the acknowledged report carried, and only if that report is still the session's staged one. `flush` (the immediate form) stages and commits in one synchronous call under the delivery lock.

What the acknowledgement confirms is the answer, not every file in it. The token covers the whole `updates` vector; the text the reader prints does not. `new_diagnostics_payload` drops a changed file whose cache source is missing, or whose URI will not map to a path, and the ack commits that file's hash anyway. The hole predates the acknowledgement and `flush` has it too; the invariant below is stated about answers so it does not claim to close it.
