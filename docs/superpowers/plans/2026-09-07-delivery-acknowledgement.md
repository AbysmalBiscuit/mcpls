...
- After the ack is read, the hook process is killed before its stdout write lands, or the host discards its stdout because the turn was aborted. The window is the return from `send_and_acknowledge`, one JSON serialization, and one write. The diagnostics in that report are not offered again until their file's hash changes. This is the irreducible residual: the owner cannot see past the hook's stdout, and nothing short of a host-side receipt would close it.
...
**Placeholder scan.** Every code step carries the code. The one conditional instruction (Task 3 Step 1, the borrow of `request` after its push) gives both forms.
...
1. **Does Claude Code kill an in-flight hook process when the user interrupts a turn?** The plan treats that as the everyday trigger. It is not verified against the host. If the host lets hooks finish, the remaining triggers are the owner stalling past 1500 ms and the host's own hook timeout, both rarer. The fix is the same either way; only the urgency argument changes.

2. **Should `Response::Ack` say whether anything was committed?** The plan answers `Ack` regardless, because the client cannot act on the difference. A `committed: bool` would let `mcpls hook doctor` or a log line distinguish a stale acknowledgement from a live one. Left out under YAGNI; cheap to add later since it is an added field on a response the client already reads.

3. **Should a stage with no updates still replace an older pending?** The plan removes it (case 8), on the reasoning that a later answer saying "nothing" should not leave an older report committable. The alternative, leaving it, can only produce a spurious "no diagnostics" line, never a loss. If the implementer finds the removal complicates `stage`, leaving it is acceptable and the doc comment on `stage` should say so.

4. **Muted-file updates.** A file whose floor is `off` and whose record entry is forgotten produces an update with no report line, so an answer can carry a token and no context. The plan accepts the resulting no-op acknowledgement. Applying that one update immediately in `diff` would avoid the odd answer at the cost of a branch; the implementer may choose either, and the `token: None` assertion in the delivery tests does not cover the muted case either way.
