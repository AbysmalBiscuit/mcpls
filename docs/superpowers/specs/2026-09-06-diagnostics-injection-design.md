...
**Where it lives.** The flush machinery is on `McplsServer`'s context, and `apply_locked` is on `Translator`, which cannot reach any of it. The `#[tool]` handlers capture the current progress epoch before entering the translator write, then run the footer only after the write returns. The footer therefore waits on progress that began after that pre-write epoch, rather than treating work already in flight as evidence for this write. Results are serialised as JSON (`mcp/server.rs:54-62`), so "append" means a field, not trailing text: one wrapper type at the MCP layer carrying `#[serde(flatten)] result: T` plus `#[serde(skip_serializing_if = "Option::is_none")] new_diagnostics: Option<NewDiagnosticsResult>`, used at all three call sites. That leaves the bridge's own DTOs untouched and keeps `NewDiagnosticsResult` where it already is.
...
2. Then wait for quiet: nothing outstanding, held for `footer_quiet_ms` (200). Only progress that **began after the pre-write epoch** counts, so an indexing run already in flight when the write landed does not eat the whole cap.
...
`changed` runs the filters below and **enqueues** the surviving paths for the sweep; it does not resync inline and it does not flush. `flush` stages the session's report and answers it with a token; the record advances only when an `ack` carrying that token arrives, which the hook sends on the same connection once the answer is in hand. A report nobody acknowledges is offered again by the next `flush`, so a hook that gives up on its deadline, or dies before it has the answer, costs a repeat rather than a loss. The `ack` goes out before the hook prints, so one window stays open: a kill landing after the `ack` has left the hook and before the hook exits commits the record while the host discards the output it was collecting, and that report is not offered again until its file changes. Sending the `ack` after the print does not close it, because the host reads a killed hook's output at its exit and discards it whenever it was written. An answer with nothing to commit carries no token and gets no `ack`. The MCP tool and the footer advance the record in the flush itself: their transport is the session's own, and losing it ends the session. `flush` does not resync. `PostToolBatch` sends `changed` then `flush` on one connection. `status` serves `doctor`.

A `PostToolBatch` sends `changed` before `flush`, but the sweep and language-server analysis run asynchronously. The flush can arrive before that work finishes, so it may omit diagnostics from the batch. A later flush may contain those results once they are available. Keeping the sweep off the connection lets changes coalesce without making each hook wait for resync and analysis.

If a handler has not finished within `op_deadline_ms`, the owner returns a deadline error. The spawned work continues in the background, but the response does not establish whether it will succeed or whether a later flush will contain diagnostics. The sweep takes per-path locks outside the connection task.
